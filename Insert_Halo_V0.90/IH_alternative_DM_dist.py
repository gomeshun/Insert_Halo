...
class Insert_Halo_SIDM_Distribution():
    ...
    def lensing_quantities(self, density_profiles_3d, kpc_per_arcsec, sigma_crit, gdecom_P = 8, ngaussian = 15, add_mass_sheet_correction = False, kwargs_mass_sheet = {},
                          return_proj_dens_approx = False):
        """
        ...
        The kpc_per_arcsec should be the interpolation function found in the geometry_and_massfunction class
        Likewise the sigma_crit should be the critical lensing density function sigma_crit_lens found in the Insert_Halo_Grav_Lensing class
        """
        kpc_per_arcsec_interp = kpc_per_arcsec()
        redshifts = []
        for halo in self.field_halos:
            redshifts.append(halo.z)

        # List of all redshifst at which pyHalo generated halo masses:
        zu = np.unique(redshifts)
        # Just to make sure the redshifts are sorted:
        zu = np.array(sorted(zu))

        sigma_crit_array = []
        for z in zu:
            sigma_crit_array.append(sigma_crit(z, self.IHB.zsource))

        sigma_crit_interp = make_interp_spline(zu, sigma_crit_array)

        #density_profile = getattr(self, density_profile_3d)
        ...
        if return_proj_dens_approx:
            analytical_interp_list = []
            n_gaussians_plus = []
        ...
        for halo in self.halos:
            ...
            density_profile = density_profiles_3d[profile_params["profile"]]
            
            if not halo.is_subhalo:
                r_array, sigma = self.lens_projected_density_eff(density_profile, profile_params, np.log10(50 * profile_params["rs"]))
            else:  
                r_array, sigma = self.lens_projected_density_eff(density_profile, profile_params, np.log10(10 * profile_params["rt"]))
            proj_dens_interp = make_interp_spline(r_array, sigma)

            if not halo.is_subhalo:
                analytical_interp, analytical_interp_params = self.fit_projected_interp(r_array, proj_dens_interp(r_array), profile_params["rc"], profile_params["rs"], 1e7)
                logsigma_max = np.log10(50 * profile_params["rs"])
            else:
                analytical_interp, analytical_interp_params = self.fit_projected_interp(r_array, proj_dens_interp(r_array), profile_params["rc"], profile_params["rs"], profile_params["rt"])
            ...
            gaussian_sigmas_arcsec = gaussian_sigmas / kpc_per_arcsec_interp(halo.z)
            gaussian_amplitudes_kappa = gaussian_amplitudes / sigma_crit_interp(halo.z) * 2* np.pi * gaussian_sigmas_arcsec**2

            N_gauss_real = 0
            for amp, sig in zip(gaussian_amplitudes_kappa, gaussian_sigmas_arcsec):
                if amp > 0:
                    lens_model_list.append("GAUSSIAN")
                    kwargs_lens_list.append({"amp": amp, "sigma": sig, "center_x": halo.x, "center_y": halo.y})
                    redshift_array.append(halo.z)
                    N_gauss_real += 1
                    

            if return_proj_dens_approx:
                analytical_interp_list.append(analytical_interp)
                n_gaussians_plus.append(N_gauss_real)

            #lens_model_list.append('MULTI_GAUSSIAN')
            #redshift_array.append(halo.z)
            #profile_params = halo.profile_args
            #r_array, sigma = self.lens_projected_density(density_profile, profile_params, np.log10(10 * profile_params["rt"]))
            #proj_dens_interp = make_interp_spline(r_array, sigma)
        ...
        if return_proj_dens_approx:
            functions = {"kpc_per_arcsec": kpc_per_arcsec_interp, "sigma_crit_lens": sigma_crit_interp}
            return lens_model_list, redshift_array, kwargs_lens_list, numerical_interp, analytical_interp_list, functions, n_gaussians_plus
        else:
    ...
    def lens_projected_density_eff(self, density_profile_3d, density_profile_params, log_rmax, log_rmin = -3, Nr = 1000, los_logmin = -3, los_logmax = 3, losN = 1000):
        r_array = np.logspace(log_rmin, log_rmax, Nr)
        los_array = np.logspace(los_logmin, los_logmax, losN)
        #los_array = np.linspace(0, 10**los_logmax, losN)
        los_array_expanded = np.expand_dims(los_array, axis = -1)
        los_array_2d = np.repeat(los_array_expanded, Nr, axis = 1)
        dplos_array = np.sqrt(los_array_2d ** 2 + r_array **2)
        integrand = density_profile_3d(dplos_array, density_profile_params)
    ...
    def TNFWC_SH_tau(self, r, params):
        tau = params["rt"] / params["rs"]
        x = r / params["rs"]
        beta = params["rc"] / params["rs"]
        rho = params["rhos"] / ((x ** 4 + beta ** 4) ** (1/4) * (x + 1) ** 2) * tau**2 / (x **2 + tau**2)
        return rho
